...
void BlockCodex::init() {
	...
	s_blocks[index(BlockType::GRASS)] = BlockData(std::make_unique<NonUpdatable>(), 0, 1, 2, BlockShape::CUBE, ShaderType::SOLID , true);
	s_blocks[index(BlockType::DIRT)]  = BlockData(std::make_unique<NonUpdatable>(), 2, 2, 2, BlockShape::CUBE, ShaderType::SOLID , true);
	s_blocks[index(BlockType::STONE)] = BlockData(std::make_unique<NonUpdatable>(), 3, 3, 3, BlockShape::CUBE, ShaderType::SOLID , true);
	s_blocks[index(BlockType::SAND)]  = BlockData(std::make_unique<GravityBlockHandler>(), 4, 4, 4, BlockShape::CUBE, ShaderType::SOLID , true);
